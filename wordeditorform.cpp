...
WordEditorForm::WordEditorForm(QWidget *parent) : base(parent), ui(new Ui::WordEditorForm), model(nullptr), /*modified(false), origmodified(false),*/ ignoreedits(false)
{
    ...
    connect(gUI, &GlobalUI::dictionaryReplaced, this, &WordEditorForm::dictionaryReplaced);
...
void WordEditorForm::dictionaryReplaced(Dictionary *olddict, Dictionary* /*newdict*/, int /*index*/)
{
    if (dict == olddict)
        close();
}
