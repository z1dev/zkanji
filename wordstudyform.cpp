...
bool WordStudyForm::event(QEvent *e)
{
    ...
    if (e->type() == QEvent::Timer && ui->mainStack->currentWidget() != ui->suspendPage && ((QTimerEvent*)e)->timerId() == timer.timerId())
    {
        ...
        else
        {
            ...
            ui->status->setValue(timeindex, QString("%1:%2:%3").arg(hours, 2, 10, QChar('0')).arg(minutes, 2, 10, QChar('0')).arg(seconds, 2, 10, QChar('0')));
        }
    }
...
void WordStudyForm::updateDeckLabels()
{
    ...
    ui->status->setValue(2, QString("%1:%2:%3").arg(hours, 2, 10, QChar('0')).arg(minutes, 2, 10, QChar('0')).arg(seconds, 2, 10, QChar('0')));
    ui->status->setValues(0, QString::number(deck->dueSize()), QString("(%1)").arg(deck->newSize()));
    ui->status->setValues(1, QString::number(passedcount), QString("(%1)").arg(deck->failedSize()));
...
void WordStudyForm::updateStudyLabels()
{
    if (study->studySettings().method == WordStudyMethod::Gradual)
        ui->status->setValues(1, QString::number(study->dueCount()), QString("(%1)").arg(study->newCount()));
    else
